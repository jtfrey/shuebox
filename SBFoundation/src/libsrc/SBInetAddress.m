...
- (id) initWithBytes:(const void*)bytes prefixLength:(SBUInteger)prefixLength;
...
@implementation SBInetAddress(SBInetAddressPrivate)
  ...
  - (id) initWithBytes:(const void*)bytes
    prefixLength:(SBUInteger)prefixLength
  {
    return [self init];
  }
...
//
  ...
  - (SBComparisonResult) compareToInetAddress:(SBInetAddress*)anotherAddress
  {
    if ( ([self addressFamily] == [anotherAddress addressFamily]) ) {
      SBUInteger        p1 = [self prefixBitLength];
      SBUInteger        p2 = [anotherAddress prefixBitLength];
      
      if ( p1 == p2 ) {
        SBUInteger      byteLength = [self byteLength];
...
//
  ...
  - (SBUInteger) copyAddressBytes:(void*)buffer
...
//
  ...
  - (SBUInteger) copyMaskedAddressBytes:(void*)buffer
    length:(SBUInteger)length
  {
    SBUInteger        copied = [self copyAddressBytes:buffer length:length];
...
//
  ...
  - (SBData*) inetAddressAsData
  {
    ...
    if ( byteLength ) {
      SBUInteger            addressDataLen = sizeof(SBInetAddressData) + byteLength - 1;
