...
//
  ...
  - (void) setDefaultSMTPPort:(int)aPort
  {
...
//
  ...
  - (SBError*) sendRecipient:(SBString*)fromAddress
  {
    SBError*      error = nil;
    
    if ( _flags & kSMTPSessionFlag_SenderSent ) {
      if ( ! (_flags & kSMTPSessionFlag_HeadersSent) ) {
        error = [self sendSMTPCommand:"RCPT TO: %s", [fromAddress utf8Characters]];
        if ( ! error )
...
//
  ...
  - (SBError*) sendRecipients:(SBArray*)fromAddresses
  {
    SBError*          error = nil;
    unsigned int      i = 0, iMax = [fromAddresses count];
    
    while ( ! error && (i < iMax) )
      error = [self sendRecipient:[fromAddresses objectAtIndex:i++]];
    return error;
  }
    ...
    if ( _flags & kSMTPSessionFlag_RecipientSent ) {
      if ( ! (_flags & kSMTPSessionFlag_HeadersSent) ) {
        ...
        if ( ! error ) {
          ...
          id              cc = [messageProps objectForKey:SBMailerToAddressesKey];
          ...
          if ( fakeRecipient ) {
            ...
            unsigned int      count, totalCount = 0;
            //
            // To:
            //
            if ( to ) {
    ...
    mimeType:(SBString*)mimeType
  {
    ...
    if ( _flags & kSMTPSessionFlag_HeadersSent ) {
      SBString*   leadIn = [SBString stringWithFormat:
                                  "\r\n"
                                  "--%s\r\n"
                                  "Content-Type: %s; charset=UNICODE-1-1-UTF-8\r\n"
...
//
  ...
  {
    fprintf(stream, "=?UNICODE-1-1-UTF-8?B?");
