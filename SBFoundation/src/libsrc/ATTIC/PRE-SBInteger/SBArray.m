...
#define SBSmallConcreteArrayCapacity     8
#define SBMediumConcreteArrayCapacity   16
...
- (BOOL) addCapacity:(unsigned int)capacity;
...
- (id) initWithParentArray:(SBArray*)parent delta:(int)delta;
...
//
  ...
  - (id) firstObjectInCommonWithArray:(SBArray*)otherArray
  {
    ...
    unsigned int      i = 0, iMax = [self count];
...
//
    ...
    inRange:(SBRange)range
  {
    ...
    unsigned int      i = range.start, iMax = SBRangeMax(range);
...
//
  ...
  - (BOOL) isEqualToArray:(SBArray*)otherArray
  {
    ...
    unsigned int      myCount = [self count];
...
//
  ...
  - (id) initWithArray:(SBArray*)anArray
  {
    unsigned int    i = 0, iMax = [anArray count];
...
@implementation SBMutableArray(SBExtendedMutableArray)
  ...
  {
    unsigned int      i = 0, iMax = [otherArray count];
...
//
  ...
  {
    unsigned int      hiIdx = 0, hiIdxVal = 0;
    unsigned int      i, iMax = count;
    BOOL              inSeq = YES;
    ...
    if ( inSeq ) {
      ...
      do {
        unsigned int    newHiIdx = 0, newHiIdxVal = 0;
...
//
  ...
  - (id) initWithFixedCapacity:(unsigned int)capacity
  {
    ...
    inRange:(SBRange)range
  {
    ...
    if ( SBArrayBucketFindSlot(_buckets, &bucket, &range.start) ) {
      ...
      while ( bucket && bucket->used && range.length ) {
        unsigned int    count = bucket->used - range.start;
...
//
  ...
  - (unsigned int) indexOfObject:(id)anObject
  {
    ...
    while ( bucket && bucket->used ) {
      ...
      while ( bSlots < eSlots ) {
        if ( [*bSlots isEqual:anObject] )
          return (baseIndex + (eSlots - bSlots));
        bSlots++;
      }
  ...
  {
    SBArrayBucket*    bucket;
    unsigned int      baseIndex = range.start;
      ...
      do {
        if ( index < bucket->used ) {
          if ( bucket->available ) {
            ...
            memmove(bucket->slots + index + 1, bucket->slots + index, (bucket->used - index) * sizeof(id));
...
//
  ...
  {
    SBArrayBucket*      bucket = _buckets;
    unsigned int        index = range.start;
