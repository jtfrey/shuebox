...
)
{
  ...
  size_t          bytes;
  size_t          defaultBaseSize = SBMEMORYPOOL_MIN_BASESIZE;
...
)
{
  ...
  if ( ! aNode ) {
    /* Allocate a new node: */
    size_t            poolSize = aMemPool->basePoolSize;
...
)
{
  ...
  if ( aNode ) {
    if ( aNode->prevAlloc == *chunk ) {
      size_t        origSize = (aNode->topAlloc - aNode->prevAlloc);
      
      if ( origSize < bytes ) {
        size_t      delta = bytes - origSize;
