...
//
  ...
  - (BOOL) isEqual:(id)anObject
  {
    if ( [anObject isKindOf:[SBInetAddress class]] ) {
      return [self isEqualToInetAddress:anObject];
    }
    return NO;
  }
...
//
  ...
  - (SBData*) inetAddressAsData
  {
    ...
    if ( byteLength ) {
      result = [SBData dataWithCapacity:sizeof(SBInetAddressData) - 1 + byteLength];
      if ( result ) {
        SBInetAddressData   addressData;
        unsigned char       b1[byteLength];
      
        [self copyAddressBytes:b1 length:byteLength];
        ...
        [result appendBytes:&addressData length:sizeof(SBInetAddressData) - 1];
        [result appendBytes:b1 length:byteLength];
      }
    }
    return result;
  }
...
//
  ...
  {
    if ( _prefix < 32 )
      return [SBString stringWithFormat:"%hhu.%hhu.%hhu.%hhu/%u",
                          _bytes[0],
                          _bytes[1],
                          _bytes[2],
                          _bytes[3],
    ...
    return [SBString stringWithFormat:"%hhu.%hhu.%hhu.%hhu",
                        _bytes[0],
                        _bytes[1],
                        _bytes[2],
                        _bytes[3]
