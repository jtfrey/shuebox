  ...
  - (BOOL) growToByteLength:(size_t)newSize
  {
    ...
    if ( _u16Chars ) {
      if ( ( p = (UChar*) realloc(_u16Chars, newSize + sizeof(UChar)) ) ) {
        ...
        bzero(p + _byteLength, (newSize + sizeof(UChar)) - _byteLength);
      }
    } else {
...
//
  ...
  {
    if ( range.start >= _charLength )
      return;
    
    if ( SBRangeMax(range) > _charLength ) {
      range.length = _charLength - range.start;
    }
    
    if ( _u16Chars ) {
      UChar*          c = 0;
      size_t          newLen = _charLength - (ssize_t)(range.length - altLen);
...
//
  ...
  {
    return [[[SBString alloc] initWithUTF8String:cString length:length] autorelease];
  }
...
//
  ...
  + (SBString*) stringWithUncopiedCharacters:(UChar*)characters
    length:(int)length
    freeWhenDone:(BOOL)freeWhenDone
  {
    return [[[SBString alloc] initWithUncopiedCharacters:characters length:length freeWhenDone:freeWhenDone] autorelease];
  }
...
//
    ...
    length:(size_t)length
  {
    if ( self = [self init] ) {
      ...
      u_strFromUTF8(NULL, 0, &u16Count, cString, ( (length == (size_t)-1) ? -1 : length ), &uerr);
      if ( u16Count ) {
    ...
    freeWhenDone:(BOOL)freeWhenDone
  {
    if ( self = [self init] ) {
      if ( characters && length ) {
        ...
        _charLength = length;
        _flags.doNotFreeWhenDone = ! freeWhenDone;
      }
    }
    return self;
  }
...
//
  ...
  - (void) dealloc
  {
    if ( _u8Chars )
      [_u8Chars release];
    if ( _u16Chars && ! _flags.doNotFreeWhenDone )
      free(_u16Chars);
    [super dealloc];
  }
...
//
  ...
  - (void) summarizeToStream:(FILE*)stream
  {
    ...
    fprintf(stream, "[ hash = %08x | bytes = %ld | chars = %ld ] {\n", [self hash], _byteLength, _charLength);
    if ( _u16Chars )
      u_fprintf(__SBStringStdout, "  %S\n", _u16Chars);
    fprintf(stream,"}\n");
  }
...
@implementation SBMutableString
  ...
  + (SBMutableString*) mutableString
  {
    return [[[SBMutableString alloc] init] autorelease];
  }
  
//

  + (SBMutableString*) mutableStringWithUTF8String:(const char*)cString
  {
    return [[[SBMutableString alloc] initWithUTF8String:cString] autorelease];
  }
...
//
  ...
  + (SBMutableString*) mutableStringWithUTF8String:(const char*)cString
    length:(size_t)length
  {
    return [[[SBMutableString alloc] initWithUTF8String:cString length:length] autorelease];
  }
  
//

  + (SBMutableString*) mutableStringWithCharacters:(UChar*)characters
    length:(int)length
  {
    return [[[SBMutableString alloc] initWithCharacters:characters length:length] autorelease];
  }
  
//

  + (SBMutableString*) mutableStringWithString:(SBString*)aString
  {
    return [[[SBMutableString alloc] initWithString:aString] autorelease];
  }

//

  + (SBMutableString*) mutableStringWithFormat:(const char*)format,...
  {
    SBMutableString*    newObj = nil;
    va_list             vargs;
    
    va_start(vargs, format);
    newObj = [[[SBMutableString alloc] initWithFormat:format arguments:vargs] autorelease];
    va_end(vargs);
...
//
  ...
  + (SBMutableString*) mutableStringWithBytes:(const void*)bytes
    count:(int)count
    encoding:(const char*)encoding
  {
    return [[[SBMutableString alloc] initWithBytes:bytes count:count encoding:encoding] autorelease];
  }
  ...
  - (void) appendFormat:(const char*)format, ...
  {
    ...
    if ( charLen ) {
      ...
      do {
        ...
        if ( appendThis ) {
          UChar*    rAppendThis = realloc(appendThis, sizeof(UChar) * charLen);
          ...
          if ( ! (appendThis = malloc( sizeof(UChar) * charLen )) ) { 
            free(appendThis);
            return;
          }
        }
...
//
  ...
  {
    SBMutableString*   result = [SBMutableString mutableStringWithString:self];
