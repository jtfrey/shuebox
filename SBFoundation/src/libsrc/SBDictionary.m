...
#define SBSmallBasicHashPairCapacity      16
#define SBSmallBasicHashTableCapacity     29

#define SBMediumBasicHashPairCapacity     32
#define SBMediumBasicHashTableCapacity    57
...
typedef struct _SBBasicHashTable {
  BOOL                freeWhenDone;
  SBUInteger          keyCapacity;
  SBUInteger          keyCount;
  id*                 keys;
  SBUInteger          tableCapacity;
  SBHashPair*         table;
} SBBasicHashTable;
...
)
{
  ...
  SBUInteger        tableSize = SBHashTableMultiplier * keyCount;
  SBUInteger        byteSize = sizeof(SBBasicHashTable) + keyCount * sizeof(id) + tableSize * sizeof(SBHashPair); 
...
- (id) initWithKeys:(id*)keys count:(SBUInteger)count;
...
- (id) initWithTable:(SBHashPair*)table count:(SBUInteger)count;
...
@implementation SBConcreteDictionary : SBDictionary
  ...
  - (id) initWithObjects:(SBArray*)objects
    forKeys:(SBArray*)keys
  {
    SBUInteger        i = 0, iMax = [objects count];
...
//
  ...
  - (void) summarizeToStream:(FILE*)stream
  {
    ...
    fprintf(
        stream,
        " ( capacity: keys = " SBUIntegerFormat " | table = " SBUIntegerFormat " ) {\n",
        myTable->keyCapacity,
    ...
    while ( i < myTable->tableCapacity ) {
      ...
      if ( (key = myTable->table[i].key) ) {
        if ( [key isKindOf:[SBString class]] ) {
          ...
          fprintf(stream, "    %s@%p[" SBUIntegerFormat "] = ", [key name], key, [key referenceCount]);
        }
        ...
        if ( [object isKindOf:[SBString class]] ) {
          ...
          fprintf(stream, "%s@%p[" SBUIntegerFormat "]\n", [object name], object, [object referenceCount]);
        }
      }
      i++;
    }
...
//
  ...
  - (void) removeAllObjects
  {
    ...
    SBUInteger          i = 0, iMax = [keys count];
...
//
  ...
  {
    SBUInteger          i = 0, iMax = [keyArray count];
...
)
{
  ...
  SBInteger                 bytes = sizeof(SBMutableHashPairPool);
  SBUInteger                tableCapacity = SBHashTableMultiplier * pairCapacity;
...
@interface SBConcreteMutableDictionary : SBMutableDictionary
{
  ...
  SBUInteger                    _count;
  SBUInteger                    _pairCapacity;
  SBUInteger                    _tableCapacity;
  SBUInteger                    _hash;
  struct {
...
//
  ...
  - (void) dealloc
  {
    ...
    // the pool itself:
    while ( _pools ) {
      SBUInteger        i = 0, iMax = _pools->tableCapacity;
...
//
  ...
  - (void) summarizeToStream:(FILE*)stream
  {
    ...
    SBUInteger                  additions = _stats.collisions + _stats.nonCollisions;
    ...
    fprintf(
        ...
        "  capacity:\n"
        "    keys = " SBUIntegerFormat "\n"
        "    table = " SBUIntegerFormat "\n"
        "  stats:\n"
        "    collisions = %hu\n"
        "    rate = %0.1lf%%\n"
        ") {\n",
    ...
    while ( pool ) {
      SBUInteger      i = 0, iMax = pool->tableCapacity;
          ...
          while ( pair ) {
            if ( (pair->key) ) {
              if ( [pair->key isKindOf:[SBString class]] ) {
                ...
                fprintf(stream, "    %s@%p[" SBUIntegerFormat "] = ", [pair->key name], pair->key, [pair->key referenceCount]);
              }
              ...
              if ( [pair->object isKindOf:[SBString class]] ) {
                ...
                fprintf(stream, "%s@%p[" SBUIntegerFormat "]\n", [pair->object name], pair->object, [pair->object referenceCount]);
              }
            }
            pair = pair->link;
          }
...
//
  ...
  - (BOOL) increaseCapacity
  {
    ...
    if ( SBMutableHashPairPoolAlloc(&_topPool, ( delta ? delta : 32 ) ) ) {
      ...
      SBUInteger                newCapacity = [self tableCapacity];
      
      _stats.collisions = _stats.nonCollisions = 0;
      
      while ( basePair ) {
        SBUInteger              index = [basePair->key hash] % newCapacity;
...
//
  ...
  {
    if ( _pools ) {
      SBUInteger            index = [aKey hash] % [self tableCapacity];
      SBMutableHashPair**   tableSlot = SBMutableHashPairPoolFindTableIndex(_pools, index);
...
//
    ...
    notFoundMarker:(id)marker
  {
    ...
    if ( count && _pools ) {
      ...
      // Walk the table:
      while ( pool ) {
        SBUInteger              iMax = pool->tableCapacity;
        ...
        while ( i < iMax ) {
          if ( pool->table[i] ) {
            ...
            while ( pair ) {
              SBUInteger        j = [keys indexOfObject:pair->key];
...
//
  ...
  - (void) setObject:(id)anObject
    forKey:(id)aKey
  {
    SBUInteger              hash = [aKey hash];
    
    if ( _pools ) {
      // Does the key already occur in the table?
      SBUInteger            index = hash % [self tableCapacity];
      SBMutableHashPair**   tableSlot = SBMutableHashPairPoolFindTableIndex(_pools, index);
...
)
{
  ...
  while ( (pairIndex < maxPairs) && (line = fgetln(fptr, &lineLen)) ) {
    char*           base = line;
    SBUInteger      keyStart = 0, keyEnd = 0;
    SBUInteger      valStart = 0, valEnd = 0;
...
@implementation SBDictionary(SBStringPairFiles)
  ...
  + (id) dictionaryWithStringPairFile:(SBString*)path
  {
    ...
    if ( fptr = [[SBFileManager sharedFileManager] openPath:path asCFileStreamWithMode:"r"] ) {
      SBUInteger      pairs = SBDictionaryCountStringPairs(fptr);
