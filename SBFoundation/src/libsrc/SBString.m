...
SBString* SBStringSharedMemPoolKey = @"SBStringSharedMemPoolKey";

SBMemoryPoolRef
__SBStringGetSharedMemPool()
{
  SBMutableDictionary*    threadProps = [[SBThread currentThread] properties];
  
  if ( threadProps ) {
    SBValue*      memPoolWrapper = [threadProps objectForKey:SBStringSharedMemPoolKey];
    
    if ( ! memPoolWrapper ) {
      SBMemoryPoolRef     newPool = SBMemoryPoolCreate(0);
      
      if ( newPool ) {
        [threadProps setObject:[SBValue valueWithPointer:newPool] forKey:SBStringSharedMemPoolKey];
        return newPool;
      }
    } else {
      return (SBMemoryPoolRef)[memPoolWrapper pointerValue];
    }
  }
  return NULL;
}
...
void
__SBStringDestroySharedMemPool()
{
  SBMutableDictionary*    threadProps = [[SBThread currentThread] properties];
  ...
  if ( threadProps ) {
    ...
    if ( memPoolWrapper )
      SBMemoryPoolRelease([memPoolWrapper pointerValue]);
  }
}
...
#  define SBSTRING_HASH_MIX64(a,b,c) \
    { \
      a -= b; a -= c; a ^= (c>>43); \
      b -= c; b -= a; b ^= (a<<9); \
      c -= a; c -= b; c ^= (b>>8); \
...
)
{
  UChar                 data[12];
  char*                 k;
  SBUInteger            iMax = [aString length];
  SBUInteger            byteLength = sizeof(UChar) * iMax;
  register SBUInteger   a, b, c;
  SBUInteger            i = 0, j;
  ...
  while ((iMax - i) >= 12)
  {
    // Get the next 12 UTF-16 characters:
    j = 0;
    while ( (i < iMax) && (j < 12) )
      data[j++] = [aString characterAtIndex:i++];
    k = (char*)data;
    
    a += (k[0]        +(SBUInteger)k[ 1]<< 8)+(SBUInteger)k[ 2]<<16)+(SBUInteger)k[ 3]<<24)
     +(SBUInteger)k[4 ]<<32)+(SBUInteger)k[ 5]<<40)+(SBUInteger)k[ 6]<<48)+(SBUInteger)k[ 7]<<56));
    b += (k[8]        +(SBUInteger)k[ 9]<< 8)+(SBUInteger)k[10]<<16)+(SBUInteger)k[11]<<24)
     +(SBUInteger)k[12]<<32)+(SBUInteger)k[13]<<40)+(SBUInteger)k[14]<<48)+(SBUInteger)k[15]<<56));
    c += (k[16]       +(SBUInteger)k[17]<< 8)+(SBUInteger)k[18]<<16)+(SBUInteger)k[19]<<24)
     +(SBUInteger)k[20]<<32)+(SBUInteger)k[21]<<40)+(SBUInteger)k[22]<<48)+(SBUInteger)k[23]<<56));
    SBSTRING_HASH_MIX64(a,b,c);
  }

  /*------------------------------------- handle the last 23 bytes */
  c += byteLength;
  
  // Get the remaining UTF-16 characters:
  j = 0;
  while ( i < iMax )
    data[j++] = [aString characterAtIndex:i++];
  k = (char*)data;
  switch(j * 2)              /* all the case statements fall through */
  {
...
)
{
  UChar           data[2];
  char*           k;
  SBUInteger      i = 0, j, iMax = [aString length];
  SBUInteger      byteLength = sizeof(UChar) * iMax;
  ...
  /* Main loop */
  while ( byteLength-- > 0 ) {
    data[0] = [aString characterAtIndex:i++];
    data[1] = [aString characterAtIndex:i++];
    hash += data[0];
    tmp = (data[1] << 11) ^ hash;
  ...
  /* Handle end cases */
  if ( i < iMax ) {
    hash += [aString characterAtIndex:i++];
    hash ^= hash << 11;
    hash += hash >> 17;
  }
...
)
{
  ...
  UCharIterator             iPrimary, iSecondary;
  int32_t                   lPrimary, lSecondary;
  SBMemoryPoolRef           pool = __SBStringGetSharedMemPool();
  ...
  // We need a memory pool for scratch space:
  if ( pool == NULL )
    return SBOrderSame;
...
SBString* SBStringSharedSearchObjKey = @"SBStringSharedSearchObjKey";

SBUnicodeSearch*
__SBStringGetSharedSearchObj()
{
  SBMutableDictionary*    threadProps = [[SBThread currentThread] properties];
  
  if ( threadProps ) {
    SBUnicodeSearch*      searchObj = [threadProps objectForKey:SBStringSharedSearchObjKey];
    
    if ( ! searchObj ) {
      if ( (searchObj = [[SBUnicodeSearch alloc] init]) ) {
        [threadProps setObject:searchObj forKey:SBStringSharedSearchObjKey];
        [searchObj release];
      }
    }
    return searchObj;
...
@implementation SBUnicodeSearch
  ...
  + (SBUnicodeSearch*) defaultUnicodeSearch
  {
    return __SBStringGetSharedSearchObj();
  }
...
- (BOOL) preflightConvertFromBytes:(const void*)bytes byteCount:(SBUInteger)byteCount charCount:(SBUInteger*)charCount;
- (BOOL) preflightConvertFromChars:(const UChar*)chars charCount:(SBUInteger)charCount byteCount:(SBUInteger*)byteCount;

- (BOOL) convertToChars:(UChar*)chars charCount:(SBUInteger)charCount fromBytes:(const void*)bytes byteCount:(SBUInteger)byteCount;
- (BOOL) convertToBytes:(void*)bytes byteCount:(SBUInteger)byteCount fromChars:(const UChar*)chars charCount:(SBUInteger)charCount actualByteCount:(SBUInteger*)actualByteCount;
...
//
  ...
  - (BOOL) preflightConvertFromBytes:(const void*)bytes
    byteCount:(SBUInteger)byteCount
    charCount:(SBUInteger*)charCount
  {
    UErrorCode    icuErr = U_ZERO_ERROR;
    SBUInteger    count;
...
//
  ...
  - (BOOL) convertToBytes:(void*)bytes
    ...
    charCount:(SBUInteger)charCount
    actualByteCount:(SBUInteger*)actualByteCount
  {
    UErrorCode    icuErr = U_ZERO_ERROR;
    SBUInteger    outByteCount;
...
+ (void) threadWillExit:(SBNotification*)notify;

- (id) initWithCapacity:(SBUInteger)charCount;
- (id) initWithUncopiedCharacters:(UChar*)characters length:(SBUInteger)length freeWhenDone:(BOOL)freeWhenDone;
...
- (BOOL) growToSize:(SBUInteger)charCapacity;
...
@implementation SBString
  ...
  + initialize
  {
    if ( self == [SBString class] ) {
      ...
      // Watch for thread exit notifications so we can cleanup per-thread
      // memory pools, etc:
      [[SBNotificationCenter defaultNotificationCenter] addObserver:self selector:@selector(threadWillExit:) identifier:SBThreadWillExitNotification object:nil];
    }
  }
...
//
  ...
  - (SBUInteger) hash
  {
    return __SBStringUTF8Hash(self);
  }
...
//
  ...
  - (UChar) characterAtIndex:(SBUInteger)index
  {
    return (UChar)0;
  }
...
//
  ...
  - (id) initWithData:(SBData*)data
    encoding:(const char*)encoding
  {
    return [self initWithBytes:[data bytes] count:[data length] encoding:encoding];
  }
...
//
  ...
  - (SBString*) substringFromIndex:(SBUInteger)from
  {
    return [self substringWithRange:SBRangeCreate(from, [self length] - from)];
  }
  - (SBString*) substringToIndex:(SBUInteger)to
  {
...
//
  ...
  {
    UChar32*      result = NULL;
    SBUInteger    utf32Len = [self utf32Length];
...
//
  ...
  - (UChar32) utf32CharacterAtIndex:(SBUInteger)index
  {
...
//
  ...
  - (const unsigned char*) utf8Characters
  {
    ...
    SBUInteger              utf8Len = [self utf8Length];
...
//
  ...
  {
    UCharIterator       iPrimary, iSecondary;
    SBUInteger          lPrimary, lSecondary;
    BOOL                result = NO;
...
//
  ...
  {
    SBString*         result = nil;
    SBUInteger        myLength = [self length];
    SBUInteger        itsLength = [aString length];
...
//
  ...
  - (void) summarizeToStream:(FILE*)stream
  {
    ...
    fprintf(stream, "[ hash = %08x | length = " SBUIntegerFormat " ] {\n  ", [self hash], [self length] );
...
//
  ...
  - (long long int) longLongIntValue
  {
    SBSTRING_AS_UTF8_BEGIN(self)
      ...
      char*          endptr;
      long long int  result = strtoll(self_utf8, &endptr, 10);
...
@implementation SBMutableString(SBMutableStringCreation)
  ...
  + (id) stringWithCapacity:(SBUInteger)charCount
  {
    return [[[self alloc] initWithCapacity:charCount] autorelease];
  }
...
//
  ...
  + (id) stringWithFixedCapacity:(SBUInteger)maxCharacters
  {
...
//
  ...
  - (id) initWithFixedCapacity:(SBUInteger)maxCharacters
  {
    return [super init];
  }    
...
//
  ...
  - (void) appendFormat:(const char*)format, ...
  {
    ...
    //
    SBUInteger    charLen = strlen(format);
...
//
  ...
  - (id) initWithCapacity:(SBUInteger)charCount
  {
    SBAssert1(charCount < INT_MAX, "String is too long: " SBUIntegerFormat, charCount);
    if ( self = [super init] ) {
      if ( charCount ) {
...
//
  ...
  - (id) initWithCharacters:(UChar*)characters
    length:(SBUInteger)length
  {
    SBAssert1(length < INT_MAX, "String is too long: " SBUIntegerFormat, length);
...
//
  ...
  - (id) initWithBytes:(const void*)bytes
    ...
    encoding:(const char*)encoding
  {
    SBAssert1(count < INT_MAX, "String is too long: " SBUIntegerFormat, count);
...
//
    ...
    range:(SBRange)searchRange
  {
    ...
    SBUInteger        searchLen = [aString length];
...
//
  ...
  + (id) stringWithCapacity:(SBUInteger)charCount
  {
    return [[[SBConcreteMutableString alloc] initWithCapacity:charCount] autorelease];
  }
...
//
  ...
  - (id) initWithCapacity:(SBUInteger)charCount
  {
    SBAssert1(charCount < INT_MAX, "Capacity is too large: " SBUIntegerFormat, charCount);
    if ( self = [super init] ) {
      if ( charCount ) {
        if ( (_u16Chars = (UChar*)objc_calloc(charCount + 1, sizeof(UChar))) ) {
          _capacity = charCount;
...
//
  ...
  - (id) initWithFixedCapacity:(SBUInteger)maxCharacters
  {
    SBAssert1(maxCharacters < INT_MAX, "Capacity is too large: " SBUIntegerFormat, maxCharacters);
    if ( self = [super init] ) {
      if ( maxCharacters ) {
...
//
  ...
  - (BOOL) growToSize:(SBUInteger)charCapacity
  {
    ...
    SBAssert1(charCapacity < INT_MAX, "Capacity would be too large: " SBUIntegerFormat, charCapacity);
    ...
    length:(SBUInteger)length
  {
    ...
    if ( ! _flags.flexCapacity ) {
      if ( _length - range.length + length > _capacity ) {
        if ( _capacity < (_length - range.length) )
          return;
...
//
  ...
  - (UChar*) utf16Characters
  {
    ...
    if ( ! _nulTerminatedU16Chars && length ) {
      _nulTerminatedU16Chars = [[SBMutableData alloc] initWithCapacity:(length + 1) * sizeof(UChar)];
...
//
  ...
  - (long long int) longLongIntValue
  {
    ...
    char*           endptr;
    long long int   result = strtoll(iData->byGCC.s, &endptr, 10);
...
//
  ...
  {
    SBUInteger    length = [self length], start = length;
