...
@interface SBFileHandleInputStream : SBFileInputStream
{
  SBFileHandle*         _fileHandle;
}
...
- (id) initWithHost:(SBHost*)aHost port:(SBUInteger)port;
...
//
  ...
  {
    return [[[SBFileHandleInputStream alloc] initWithFileHandle:aFileHandle] autorelease];
  }
...
//
  ...
  - (id) initWithFileHandle:(SBFileHandle*)aFileHandle
  {
    if ( [self class] == [SBInputStream class] ) {
      [self release];
      self = [[SBFileHandleInputStream alloc] initWithFileHandle:aFileHandle];
    } else {
      self = [self init];
    }
  }
...
//
  ...
  - (SBUInteger) read:(void*)buffer
    maxLength:(SBUInteger)length
  {
    return 0;
  }
...
@implementation SBFileHandleInputStream


  - (id) initWithFileHandle:(SBFileHandle*)aFileHandle
  {
    if ( aFileHandle && [aFileHandle isReadable] ) {
      if ( (self = [super initWithFileDescriptor:[aFileHandle fileDescriptor] closeWhenDone:NO]) ) {
...
//
  ...
  - (SBUInteger) write:(void*)buffer
    length:(SBUInteger)length
  {
    SBUInteger        actualWrite = 0;
    
    if ( (_streamStatus == SBStreamStatusOpen) && length ) {
      SBUInteger      startLength = [_outputData length];
        ...
        switch ( [hostIP addressFamily] ) {
          ...
          case kSBInetAddressIPv4Family: {
            ...
            if ( [hostIP setSockAddr:(struct sockaddr*)&sockAddress byteSize:sizeof(sockAddress)] ) {
              sockAddress.sin_port = _port; htons(sockAddress.sin_port);
          ...
          case kSBInetAddressIPv6Family: {
            ...
            if ( [hostIP setSockAddr:(struct sockaddr*)&sockAddress byteSize:sizeof(sockAddress)] ) {
              sockAddress.sin6_port = _port; htons(sockAddress.sin6_port);
