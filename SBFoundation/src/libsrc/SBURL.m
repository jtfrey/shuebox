  ...
  - (SBNumber*) port
  {
    if ( ! _cache[kSBURLInternetPortIndex] ) {
      if ( SBRangeEmpty(_parts[kSBURLInternetPortPart]) ) {
        ...
        unsigned int        port = 0;
        UChar               c;
        SBUInteger          i = _parts[kSBURLInternetPortPart].start, iMax = SBRangeMax(_parts[kSBURLInternetPortPart]);
    ...
    if ( ! error ) {
      ...
      if ( contentType ) {
        ...
        if ( ! SBRangeEmpty(charsetRange) ) {
          SBUInteger        i = SBRangeMax(charsetRange), iMax = [contentType length];
